...
class ResponseGeneratorRecipeQuery:
    ...
    def execute(self, state):
        ...
        if getattr(state.user_attributes, 'recipe_filter', None) is not None:
            recipe_taskmap_result = self._get_filtered_recipe(state, recipe_taskmap_result)
    ...
    def _get_filtered_recipe(self, recipe_taskmap_result):
        '''
    ...
    def _get_recipe_query_speak_output(self, state, recipe_taskmap_result, is_apl_supported, range_min, range_max):
        ...
        if num_results == 0:
            ...
            speak_output = f"I found a couple of options from WholeFoods! Say the option number to select it."
            if getattr(state.user_attributes, 'more_results', False):
    ...
    def _get_recipe_query_document(self, state, recipe_taskmap_result, is_apl_supported, range_min, range_max):
        ...
        if not is_apl_supported:
            return self._get_recipe_query_speak_output(recipe_taskmap_result, is_apl_supported, range_min, range_max)
        else:
            ...
            # speak_output
            speak_output = self._get_recipe_query_speak_output(recipe_taskmap_result, is_apl_supported, range_min, range_max)
            document.data = data
